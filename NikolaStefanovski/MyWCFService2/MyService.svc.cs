using System;
using System.Collections.Generic;
using System.Linq;
using System.Runtime.Serialization;
using System.ServiceModel;
using System.ServiceModel.Web;
using System.Text;

namespace MyWCFService2
{
    // NOTE: You can use the "Rename" command on the "Refactor" menu to change the class name "Service1" in code, svc and config file together.
    // NOTE: In order to launch WCF Test Client for testing this service, please select Service1.svc or Service1.svc.cs at the Solution Explorer and start debugging.
    public class Service1 : IMyService
    {
        //public string GetAccountBalance(AccountService account)
        //{
           // return account.TheAccount.Balance.Amount + " " + account.TheAccount.Balance.Currency;
       // }
   
        public StringMagic DoSomeMagicToString(StringMagic theString)
        {
            if (theString == null)
            {
                throw new ArgumentNullException("StringMagic");
            }

            StringMagic result = new StringMagic();
            result.StringValue = theString.StringValue.ToLower();

            if (theString.Small)
            {
                result.Small = true;
                result.StringValue = theString.StringValue.ToLower();
            }
            else
            {
                result.Small = false;
                result.StringValue = theString.StringValue.ToUpper();
            }

            return result;
        }
    }
}
